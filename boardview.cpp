...
void BoardView::drawBoardFrame() {
    ...
    QString numberTitles[] = {"8", "7", "6", "5", "4", "3", "2", "1"};

    // QString lettersTitles[] = {"H", "G", "F", "E", "D", "C", "B", "A"};
...
void BoardView::drawCheckWarningTextItems() {
    ...
    double titleXPosition = 650;//startXPosition + (BoardField::defaultWidthHeight*numberOfRowsColumns)/2 - checkWarningTitleTextItem->boundingRect().width()/2;
    double titleYPosition = 550;//startYPosition + (BoardField::defaultWidthHeight*numberOfRowsColumns) + 40;
    checkWarningTitleTextItem->setPos(titleXPosition, titleYPosition);
    checkWarningTitleTextItem->setOpacity(0);

    checkWarningDescriptionTextItem = Utils::createTextItem("You cannot make any move that places your \n\t       own king in check", 18, Constants::defaultTextColor, this);
    double descriptionXPosition = 650;//startXPosition + (BoardField::defaultWidthHeight*numberOfRowsColumns)/2 - checkWarningDescriptionTextItem->boundingRect().width()/2;
    double descriptionYPosition = 580;//startYPosition + (BoardField::defaultWidthHeight*numberOfRowsColumns) + 60;
